...
import { Model } from "../Model/RYSeDt.Model.js";
...
export default class RYSeDt {
        ...
        Base.getSession()
            .then(response => {
                ...
                if (casename) {
                    ...
                    const RYSeDtdata = Osemosys.getData(casename, "RYSeDt.json");
                    promise.push(RYSeDtdata);
                    return Promise.all(promise);
                } else {
                    MessageSelect.init(RYSeDt.refreshPage.bind(RYSeDt));
                }
            })
            .then(data => {
                let [casename, genData, PARAMETERS, RYSeDtdata] = data;
                let model = new Model(casename, genData, RYSeDtdata, group, PARAMETERS, param);
                this.initPage(model);
    ...
    static refreshPage(casename) {
        Base.setSession(casename)
            ...
            .then(data => {
                let [casename, genData, PARAMETERS, RYSeDtdata] = data;
                let model = new Model(casename, genData, RYSeDtdata, 'RYSeDt', PARAMETERS, PARAMETERS['RYSeDt'][0]['id']);
                this.initPage(model);
                this.initEvents(model);
            })
            .catch(error => {
    ...
    static initEvents(model) {
        ...
        $("#casePicker").on('click', '.selectCS', function (e) {
            ...
            RYSeDt.refreshPage(casename);
            Message.smallBoxConfirmation("Confirmation!", "Model " + casename + " selected!", 3500);
        });

        $("#osy-saveRYSeDtdata").off('click');
        $("#osy-saveRYSeDtdata").on('click', function (event) {
            event.preventDefault();
            event.stopImmediatePropagation();
            let param = $("#osy-ryt").val();
            let rytData = $divGrid.jqxGrid('getboundrows');
            let data = JSON.parse(JSON.stringify(rytData, ['ScId', 'SeId', 'DtId'].concat(model.years)));
            let saveData = {};
            ...
            Osemosys.updateData(saveData, param, "RYSeDt.json")
            .then(response => {
                Message.bigBoxSuccess('Model message', response.message, 3000);
                //sync S3
                if (Base.AWS_SYNC == 1) {
                    Base.updateSync(model.casename, "RYSeDt.json");
                }
            })
            .catch(error => {
        ...
        $divGrid.bind('keydown', function (event) {
            ...
            if (!pasteEvent) {
                ...
                var SeId = $divGrid.jqxGrid('getcellvalue', rowBoundIndex, 'SeId');
                var dtId = $divGrid.jqxGrid('getcellvalue', rowBoundIndex, 'DtId');
                var ScId = $divGrid.jqxGrid('getcellvalue', rowBoundIndex, 'ScId');
                let param = $("#osy-ryt").val();

                //update grid model
                $.each(model.gridData[param], function (id, obj) {
                    if (obj.TechId == SeId && obj.DtId == dtId && obj.ScId == ScId) {
                        if (value) {
                            obj[year] = value;
                        } else {
                            obj[year] = 0;
        ...
        $("#xlsAll").click(function (e) {
            ...
            let data = JSON.parse(JSON.stringify(rytData, ['Sc', 'Se', 'Dt'].concat(model.years)));
        ...
        $("#resizeColumns").click(function () {
            if (res) {
                ...
                $divGrid.jqxGrid('autoresizecolumn', 'Se');
                $divGrid.jqxGrid('autoresizecolumn', 'Dt');
            }
            else {
