...
import { Model } from "../Model/RYTSM.Model.js";
...
export default class RYTSM {
        ...
        Base.getSession()
            .then(response => {
                ...
                if (casename) {
                    ...
                    const RYTSMdata = Osemosys.getData(casename, "RYTSM.json");
                    promise.push(RYTSMdata);
                    ...
                    // MessageSelect.init(RYTSM.refreshPage.bind(RYTSM));
                    // throw new Error('No Model selected');
                }
            })
            .then(data => {
                let [casename, genData, PARAMETERS, RYTSMdata, cases] = data;
                //console.log('RYTSMdata ', RYTSMdata)
                if (RYTSMdata[param]['SC_0'].length == 0) {
                    let er = {
                    ...
                    let model = new Model(casename, genData, RYTSMdata, group, PARAMETERS, param, cases);
                    this.initPage(model);
            ...
            .catch(error => {
                if (error.status_code == 'CaseError') {
                    ...
                    MessageSelect.activity(RYTSM.refreshPage.bind(RYTSM), error.casename);
                }
                Message.loaderEnd();
    ...
    static refreshPage(casename) {
        ...
        Base.setSession(casename)
            ...
            .then(data => {
                let [casename, genData, PARAMETERS, RYTSMdata, cases] = data;
                if (RYTSMdata[PARAMETERS['RYTSM'][0]['id']]['SC_0'].length == 0) {
                    let er = {
                        "message": 'There is no activity defined!',
                        "status_code": "ActivityError",
                        "casename": casename
                    ...
                    let model = new Model(casename, genData, RYTSMdata, 'RYTSM', PARAMETERS, PARAMETERS['RYTSM'][0]['id'], cases);
                    this.initPage(model);
                    this.initEvents(model);
                }
            })
    ...
    static initEvents(model) {
        ...
        $("#casePicker").on('click', '.selectCS', function (e) {
            ...
            RYTSM.refreshPage(casename);
            Html.updateCasePicker(casename);
            Message.smallBoxConfirmation("Confirmation!", "Model " + casename + " selected!", 3500);
        });

        $("#osy-saveRYTSMdata").off('click');
        $("#osy-saveRYTSMdata").on('click', function (event) {
            event.preventDefault();
            event.stopImmediatePropagation();
            let param = $("#osy-ryt").val();
            let rytData = $divGrid.jqxGrid('getboundrows');
            let data = JSON.parse(JSON.stringify(rytData, ['ScId', 'TechId', 'StgId', 'MoId'].concat(model.years)));
            ...
            Osemosys.updateData(saveData, param, "RYTSM.json")
                .then(response => {
                    Message.bigBoxSuccess('Model message', response.message, 3000);
                    //sync S3
                    if (Base.AWS_SYNC == 1) {
                        Base.updateSync(model.casename, "RYTSM.json");
                    }
                })
                .catch(error => {
        ...
        $('#osy-ryt').on('change', function () {
            Message.clearMessages();
            if (model.RYTSMdata[this.value]['SC_0'].length === 0) {
                MessageSelect.activity(RYTSM.refreshPage.bind(RYTSM), model.casename);
        ...
        $("#osy-removeScData").on('click', function (e) {
            ...
            //     if (obj.Sc == sc && obj.Stg == comm && obj.Tech == tech) {
